    ...
    fs.readdir(
      ...
      async (err, node_modules) => {
        ...
        const uploads = _.filter(node_modules, node_module => {
          ...
          return (
            _.startsWith(node_module, 'strapi-provider-upload') ||
            _.startsWith(node_module, 'strapi-upload')
          );
        });
        ...
        node_modules
          ...
          .forEach(orga => {
            const node_modules = fs.readdirSync(
              path.join(basePath, 'node_modules', orga)
            );
        ...
        try {
          ...
          strapi.log.warn(
            `Please install strapi-provider-upload-${
              config.provider
            } --save in ${path.join(

...
module.exports = {
  ...
  getNature: (association, key, models, currentModelName) => {
    try {
      ...
      if (
        ...
        if (!relatedAttribute) {
          throw new Error(
            `The attribute \`${association.via}\` is missing in the model ${_.upperFirst(
        ...
        _.forIn(models, model => {
          _.forIn(model.attributes, attribute => {
            if (attribute.hasOwnProperty('via') && attribute.via === key) {
              ...
              // this use case isn't handled in the end
              else if (attribute.hasOwnProperty('model')) {
                types.other = 'modelD';
      ...
      if (types.current === 'collection' && types.other === 'morphTo') {
        ...
        // cannot get here because of previous condition
    ...
    try {
      ...
      if (globalName !== '*') {
        details = association.plugin
          ? _.get(
            strapi.plugins,
            `${association.plugin}.models.${globalName}.attributes.${association.via}`,
            {}
          )
  ...
  getVia: (attribute, association) => {
    return _.findKey(strapi.models[association.model || association.collection].attributes, {
      via: attribute,
    });
  },

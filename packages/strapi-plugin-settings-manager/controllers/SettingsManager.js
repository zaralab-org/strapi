...
module.exports = {
  ...
  databases: async ctx => {
    ...
    if (!env || _.isEmpty(_.find(Service.getEnvironments(), { name: env })))
      return ctx.badRequest(null, [
        { messages: [{ id: 'request.error.environment.unknow' }] },
      ]);
  ...
  database: async ctx => {
    ...
    if (!name || _.isEmpty(_.find(Service.getDatabases(env), { name })))
      return ctx.badRequest(null, [
        { messages: [{ id: 'request.error.database.unknow' }] },
      ]);
  ...
  get: async ctx => {
    ...
    _.has(Service, slug)
      ? ctx.send(await Service[slug](env))
      : ctx.badRequest(null, [{ messages: [{ id: 'request.error.config' }] }]);
  },
  ...
  update: async ctx => {
    ...
    if (_.has(Service, slug)) {
      ...
      return ctx.badRequest(null, [
        { messages: [{ id: 'request.error.config' }] },
      ]);
    }
    ...
    if (!_.isEmpty(validationErrors))
      return ctx.badRequest(null, Service.formatErrors(validationErrors));
    ...
    !_.isEmpty(updateErrors)
      ? ctx.badRequest(null, Service.formatErrors(updateErrors))
      : ctx.send({ ok: true });
  ...
  createLanguage: async ctx => {
    ...
    const availableLanguages =
      strapi.plugins['settings-manager'].services.languages;

    if (_.find(languages, { name: _.lowerCase(name).replace(' ', '_') }))
      return ctx.badRequest(null, [
        { messages: [{ id: 'request.error.languages.exist' }] },
      ]);
    if (!_.find(availableLanguages, { value: name }))
      return ctx.badRequest(null, [
        { messages: [{ id: 'request.error.languages.incorrect' }] },
      ]);
  ...
  deleteLanguage: async ctx => {
    ...
    if (!_.find(languages, { name }))
      return ctx.badRequest(null, [
        { messages: [{ id: 'request.error.languages.unknow' }] },
      ]);
  ...
  createDatabase: async ctx => {
    ...
    if (!name || _.find(Service.getDatabases(env), { name }))
      return ctx.badRequest(null, [
        { messages: [{ id: 'request.error.database.exist' }] },
      ]);
    ...
    params.database.connections[name].connector = Service.getClientConnector(
      params.database.connections[name].settings.client
    );
    ...
    if (
      _.isEmpty(_.keys(strapi.config.environments[env].database.connections)) ||
      _.isEmpty(strapi.config.environments[env].database.defaultConnection)
    ) {
    ...
    if (!_.isEmpty(updateErrors))
      return ctx.badRequest(null, Service.formatErrors(updateErrors));
  ...
  updateDatabase: async ctx => {
    ...
    if (params.database.connections) {
      const settings = _.assign(
        _.clone(
          strapi.config.environments[env].database.connections[name].settings
        ),
        params.database.connections[name].settings
      );
      const options = _.assign(
        _.clone(
          strapi.config.environments[env].database.connections[name].options
        ),
        params.database.connections[name].options
      );
      params = _.assign(
        _.clone(strapi.config.environments[env].database.connections[name]),
        params.database.connections[name]
      );
    ...
    if (newName && newName !== name) {
      ...
      _.forEach(strapi.models, (model, modelName) => {
        if (name === model.connection) {
          const [searchFilePath, getModelPathErrors] = Service.getModelPath(
            modelName
          );
          ...
          if (!_.isEmpty(getModelPathErrors)) {
            return ctx.badRequest(
              null,
              Service.formatErrors(getModelPathErrors)
            );
          }
    ...
    if (
      newName &&
      newName !== name &&
      strapi.config.environments[env].database.defaultConnection === name
    ) {
    ...
    const newClient = _.get(
      params,
      `database.connections.${name}.settings.client`
    );

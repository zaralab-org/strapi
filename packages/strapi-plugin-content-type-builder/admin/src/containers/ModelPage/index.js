...
export class ModelPage extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      prevProps.match.params.modelName !== this.props.match.params.modelName
    ) {
  ...
  fetchModel = props => {
    if (
      ...
      get(storeData.getContentType(), 'name') === props.match.params.modelName
    ) {
  ...
  renderAddLink = (props, customLinkStyles) => (
    <li className={customLinkStyles.pluginLeftMenuLink}>
      <div
        className={`${customLinkStyles.liInnerContainer} ${styles.iconPlus}`}
      ...
      >
        ...
        <span>
          <FormattedMessage id={`content-type-builder.${props.link.name}`} />
        </span>
      </div>
    </li>
  );
    ...
    return (
      <li className={linkStyles.pluginLeftMenuLink}>
        <NavLink
          ...
          to={`/plugins/content-type-builder/models/${props.link.name}${
    ...
    if (relationShipNumber > 0) {
      fullTitle = (
        <div className={listStyles.titleContainer}>
          ...
          />{' '}
          {relationShipNumber} <FormattedMessage id={relationShipTitle} />
        </div>
      );
    } else {
      fullTitle = (

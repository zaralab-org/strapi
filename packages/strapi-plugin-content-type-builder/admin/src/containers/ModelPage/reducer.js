...
function modelPageReducer(state = initialState, action) {
  switch (action.type) {
    case ADD_ATTRIBUTE_RELATION_TO_CONTENT_TYPE:
      return state
        .updateIn(['model', 'attributes'], list =>
          list.push(action.newAttribute, action.parallelAttribute)
        )
        .set('showButtons', true);
    case ADD_ATTRIBUTE_TO_CONTENT_TYPE:
      return state
        .updateIn(['model', 'attributes'], list =>
          list.push(action.newAttribute)
        )
        .set('showButtons', true);
    case CANCEL_CHANGES:
      return state
    ...
    case EDIT_CONTENT_TYPE_ATTRIBUTE: {
      if (action.shouldAddParralAttribute) {
        return state
          .set('showButtons', true)
          .updateIn(
            ['model', 'attributes', action.attributePosition],
            () => action.modifiedAttribute
          )
          .updateIn(['model', 'attributes'], list =>
            list.splice(
              action.attributePosition + 1,
              0,
              action.parallelAttribute
            )
          );
      }
    ...
    case EDIT_CONTENT_TYPE_ATTRIBUTE_RELATION: {
      if (action.shouldRemoveParallelAttribute) {
        return state
          ...
          .updateIn(['model', 'attributes'], list =>
            list.splice(action.parallelAttributePosition, 1)
          );
      }
      return state
        .set('showButtons', true)
        ...
        .updateIn(
          ['model', 'attributes', action.parallelAttributePosition],
          () => action.parallelAttribute
        );
    }
    case DELETE_ATTRIBUTE: {
      ...
      let showButtons =
        size(updatedContentTypeAttributes) !==
          size(state.getIn(['initialModel', 'attributes']).toJS()) ||
        size(
          differenceBy(
            state.getIn(['initialModel', 'attributes']).toJS(),
            updatedContentTypeAttributes,
            'name'
          )
        ) > 0;
      ...
      if (
        ...
        state.getIn(['initialModel', 'name'])
      ) {
        showButtons = size(get(storeData.getContentType(), 'attributes')) > 0;
      }

      if (action.shouldRemoveParallelAttribute) {
        const attributeKey = state.getIn([
          'model',
          'attributes',
          action.position,
        ]).params.key;
        ...
        return state
          ...
          .updateIn(['model', 'attributes'], list =>
            list.splice(findIndex(list.toJS(), ['name', attributeKey]), 1)
          );
      }
    ...
    case MODEL_FETCH_SUCCEEDED:
      return state
        ...
        .setIn(
          ...
          List(action.model.model.attributes)
        );
    case POST_CONTENT_TYPE_SUCCEEDED:
      return state.set(
        'postContentTypeSuccess',
        !state.get('postContentTypeSuccess')
      );

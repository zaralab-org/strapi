...
module.exports.app = async function() {
  ...
  (this.config.info.strapi = (
    get(this.config, 'info.dependencies.strapi') || ''
  ).replace(/(\^|~)/g, '')),
  ...
  this.plugins = Object.keys(this.plugins).reduce((acc, key) => {
    ...
    this.plugins[key].models = Object.keys(
      ...
      if (!this.plugins[key].models[index].globalId) {
        this.plugins[key].models[index].globalId = upperFirst(
          camelCase(`${key}-${index}`)
        );
      }
  ...
  const flattenHooksConfig = pullAll(
  ...
  this.config.middleware.settings = Object.keys(this.middleware).reduce(
    (acc, current) => {
      ...
      const currentSettings = merge(
        get(cloneDeep(this.middleware[current]), ['defaults', current], {}),
  ...
  this.config.hook.settings = Object.keys(this.hook).reduce((acc, current) => {
    // Try to find the settings in the current environment, then in the main configurations.
    const currentSettings = merge(
      get(cloneDeep(this.hook[current]), ['defaults', current], {}),
      flattenHooksConfig[current] ||
        get(this.config.currentEnvironment, ['hook', current]) ||
        get(this.config, ['hook', current])
    );
    acc[current] = !isObject(currentSettings) ? {} : currentSettings;
  ...
  // default settings
  this.config.port =
    get(this.config.currentEnvironment, 'server.port') || this.config.port;
  this.config.host =
    get(this.config.currentEnvironment, 'server.host') || this.config.host;
...
const enableHookNestedDependencies = function(
  ...
  if (isEmpty(get(flattenHooksConfig, name, true))) {
    ...
    const modelsUsed = Object.keys(assign(clone(this.api) || {}, this.plugins))
      ...
      .map(
        x => get(this.api, [x, 'models']) || get(this.plugins, [x, 'models'])
      ) // Keep models
      .filter(models => {

    ...
    try {
      ...
      try {
        ...
        try {
          ...
          strapi.log.error(
            `Impossible to write the layout file at ${layoutPath}`
          );
          strapi.log.warn(
            'This file needs to be writable to keep the documentation up-to-date.'
          );
          console.error(e);
        }
      } catch (e) {
        strapi.log.error(
          `Impossible to read OpenAPI specs at ${openAPISpecsPath}`
        );

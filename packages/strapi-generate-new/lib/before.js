  ...
  const connectionValidation = async () => {
    ...
    const asyncFn = [
      ...
      new Promise(resolve => {
        ...
        let linkNodeModulesCommand = `cd ${scope.tmpPath} && npm link ${
        ...
        shell.exec(cmd, { silent: true }, () => {
          if (scope.client.module) {
            ...
            if (
              ...
              scope.client.connector === 'strapi-hook-bookshelf'
            ) {
              let knexVersion;

              try {
                knexVersion = require(path.join(
                  `${scope.tmpPath}`,
                  '/node_modules/',
                  'knex',
                  'package.json'
              ...
              scope.additionalsDependencies[1] = `knex@${knexVersion.version ||
                'latest'}`;
            }
          }
